# 1 "C:\\Users\\matte\\Documents\\GitHub\\BatterySolder\\BatterySolder.ino"
/*
...
*/
# 40 "C:\\Users\\matte\\Documents\\GitHub\\BatterySolder\\BatterySolder.ino"
...
*/
# 105 "C:\\Users\\matte\\Documents\\GitHub\\BatterySolder\\BatterySolder.ino"
// Versione con calcolo tempo di on e stampa eventuale minuti, ore
# 107 "C:\\Users\\matte\\Documents\\GitHub\\BatterySolder\\BatterySolder.ino" 2

const int pwmPin = 9;
const int potPin = A1;
const float voltage = 12.0; // Tensione di alimentazione in Volt
const float resistance = 3.673; // Resistenza della sonda riscaldante in Ohm (in un altra sonda, 3.465)
const float massimoDC = 90; // Massimo duty cycle in percentuale

// Pin assegnati all'LCD
const int rs = 12, en = 11, d4 = 5, d5 = 4, d6 = 3, d7 = 2;
LiquidCrystal lcd(rs, en, d4, d5, d6, d7);

unsigned long previousMillis = 0; // Memorizza il tempo dell'ultimo aggiornamento
const long interval = 500; // Intervallo di aggiornamento in millisecondi
...
void loop() {
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    int potValue = analogRead(potPin); // Legge il valore del potenziometro
    int pwmValue = map(potValue, 0, 1023, 0, (int)((massimoDC / 100.0) * 255)); // Mappa il valore del potenziometro al range del PWM
    pwmValue = ((pwmValue)<(0)?(0):((pwmValue)>(255)?(255):(pwmValue))); // Assicura che pwmValue non superi 255
    analogWrite(pwmPin, pwmValue); // Imposta il valore PWM
